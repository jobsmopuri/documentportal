...
class SingleDocIngestor:
    def __init__(self,data_dir: str="data/single_document_chat",faiss_dir: str ="faiss_index"):
        try:
            ...
            self.log.info("Single Document Initalized",temp_path = str(self.data_dir), faiss_path = str(self.faiss_dir))
        except Exception as ex:
            self.log.error(f"Error initalizing SingleDocIngestor: {ex}")
            raise DocuementPortalException("Error initailizing SingleDocIngestor", sys)
    ...
    def ingest_files(self,uploaded_files):
        try:
            ...
            self.log.error(f"Document Ingestion failed: {ex}")
            raise DocuementPortalException("Error during file ingestion", sys)
        
    def _create_retriver(self,documents):
        try:
            splitter = RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=300)
            chunks = splitter.split_documents(documents)
            self.log.info("Documents split into chunks", count=len(documents))
            ...
            self.log.error(f"Retrival creation failed: {ex}")
            raise DocuementPortalException("Error creating FIASS retriver", sys)
